...
struct prop_type {
    double rch[2]; double hg[2]; double wn; double dh; double ens; double gme;
    double zgndreal; double zgndimag; double he[2]; double dl[2]; double the[2];
    int kwx; int mdp; double dist;
};
struct propv_type { double sgc; int lvar; int mdvar; int klim; };
struct propa_type {
    double dlsa; double dx; double ael; double ak1; double ak2; double aed;
    double emd; double aes; double ems; double dls[2]; double dla; double tha;
};
...
int main() {
    ...
    elev[0] = n;
    elev[1] = (dkm * 1000.0) / n;
    // Rolling terrain with delta_h ~ 300m (typical rolling hills)
    for (int i = 2; i <= n + 2; i++) {
        elev[i] = 400.0 + 150.0 * sin(i * 0.1) + 100.0 * cos(i * 0.05);
    }

    double tht_m = 50.0, rht_m = 1500.0, eps = 15.0, sgm = 0.005, eno = 301.0, frq = 1200.0;
    int klim = 5, pol = 1;
    double conf = 0.50, rel = 0.80;
    ...
    for (int mdvar_choice : {12, 1, 2, 3}) {
        ...
        printf("d=%.0fkm, mdvar=%2d, dh=%.1fm -> var_loss=%5.2f dB, total_loss=%6.2f dB, Rx dBm=%6.2f dBm (dla=%.1fkm)\n",
               prop.dist/1000.0, mdvar_choice, prop.dh, var_loss, total_loss, dBm, propa.dla/1000.0);
    }
    return 0;
}
