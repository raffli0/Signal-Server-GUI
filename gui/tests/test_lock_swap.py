...
def test_navbar_header_lock_state(qapp):
...
def test_profile_panel_lock_and_swap_state(qapp):
    panel = CloudRFPathProfilePanel()
    assert hasattr(panel, "btn_swap")
    assert hasattr(panel, "btn_lock")
    assert panel.points_locked is False
    assert "Lock" in panel.btn_lock.text()

    swap_emitted = []
    lock_emitted = []
    panel.swap_requested.connect(lambda: swap_emitted.append(True))
    panel.lock_toggled.connect(lambda val: lock_emitted.append(val))

    panel.btn_swap.click()
    assert len(swap_emitted) == 1

    panel.btn_lock.click()
    assert len(lock_emitted) == 1
...
def test_main_window_toggle_points_locked_and_guard_pick(qapp, monkeypatch):
    ...
    assert "Locked" in win.path_profile_panel.btn_lock.text()
    ...
    assert "Lock" in win.path_profile_panel.btn_lock.text()
    win._on_picked("tx", -7.0, 108.0)
...
def test_main_window_swap_tx_rx(qapp, monkeypatch):
    ...
    # Perform swap via path profile panel action
    win.path_profile_panel.btn_swap.click()
