...
class ParameterForm(QWidget):
    ...
    def load(self, d: dict) -> None:
        self.units.setCurrentText("Metric" if d.get("units") == "metric" else "Imperial")
        ...
        self.dem_res.setCurrentIndex({3: 0, 1: 1, 15: 2}.get(int(d.get("dem_resolution", 3)), 0))
        ...
        if d.get("antenna_basename"):
            self.ant_path.setText(d["antenna_basename"])
        ...
        # Load model combo
        pm = int(d.get("model_pm", 3))
        for idx in range(self.model.count()):
            if self.model.itemData(idx) == pm:
                self.model.setCurrentIndex(idx)
                break

        rel = str(d.get("reliability", 50)) + "%"
        self.reliability.setCurrentText(rel if rel in ["50%", "80%", "90%", "95%", "99%"] else "50%")
        
        ctx_pe = int(d.get("context_pe", 1))
        self.context.setCurrentIndex(0 if ctx_pe == 1 else (1 if ctx_pe == 2 else 2))
        
        self.knife.setChecked(bool(d.get("knife_edge", False)))
        if d.get("knife_edge"):
            self.diffraction.setCurrentText("Knife-edge (KED)")
        else:
            self.diffraction.setCurrentText("Off (LOS)")

        if d.get("climate_zone"):
            self.climate.setCurrentText(f"{d['climate_zone']}: " + dict(params_mod.CLIMATE_ZONES).get(int(d["climate_zone"]), ""))
        if d.get("clutter_file"):
            self.clutter_path.setText(d["clutter_file"])
        self.gc.setValue(float(d.get("ground_clutter", 0)))
        self.obstacles.setPlainText("\n".join(d.get("obstacles", [])))
        self.engine.setCurrentText(d.get("engine", "Standard"))
        self.terrain.setCurrentText("LIDAR (.asc)" if d.get("terrain_source") == "lidar" else "SDF (terrain)")
        if d.get("sdf_dir"):
            self.sdf_path.setText(d["sdf_dir"])
        if d.get("lidar_file"):
            self.lidar_path.setText(d["lidar_file"])
        self.resolution.setCurrentText(str(d.get("resolution", 1200)))
        self.radius.setValue(float(d.get("radius", 30)))
        if d.get("color_file"):
            self.color_path.setText(d["color_file"])
