...
def calculate_two_ray(
    ...
    if elev and n_samples > 1:
        for i in range(1, n_samples):
            d1 = i * sample_dist
            d2 = d_m - d1
            earth_bulge = (d1 * d2) / (2.0 * EARTH_RADIUS_EFF_M)
            h_terr = elev[2 + i] + earth_bulge
    ...
    if best_i == -1:
        ...
        best_d2 = d_m - best_d1
        best_h1 = max(1.0, h1 - (best_d1 * best_d1) / (2.0 * EARTH_RADIUS_EFF_M))
        best_h2 = max(1.0, h2 - (best_d2 * best_d2) / (2.0 * EARTH_RADIUS_EFF_M))
